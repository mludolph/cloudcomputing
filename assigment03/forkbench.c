...
int spawnChild(int start, int end)
{
	...
	if (cpid == 0) // child process
	{
		...
		// Calculate result using forkebench
		Result result = forkbench(start, end);

		// Write result to pipe and close pipe
		write(pipefd[1], &result, sizeof(struct Result));
		close(pipefd[1]);
...
Result readChild(int fd)
{
	...
	Result result = {0, 0};
	read(fd, &result, sizeof(struct Result));
	close(fd);
